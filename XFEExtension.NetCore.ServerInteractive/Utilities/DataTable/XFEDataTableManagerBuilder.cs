...
public abstract class XFEDataTableManagerBuilder
{
    ...
    /// <returns></returns>
    public XFEDataTableManagerBuilder AddTable<T>(XFEDataListTable<T> xFEDataTable) where T : IIdModel
    {
    ...
    /// <returns></returns>
    public XFEDataTableManagerBuilder AddTable<T, TP>(string tableShowName, int addPermissionLevel, int removePermissionLevel, int changePermissionLevel, int getPermissionLevel, JsonSerializerOptions? jsonSerializerOptions = null) where T : IIdModel where TP : XFEProfile => AddTable(new XFEDataListTable<T>(typeof(TP))
    {
