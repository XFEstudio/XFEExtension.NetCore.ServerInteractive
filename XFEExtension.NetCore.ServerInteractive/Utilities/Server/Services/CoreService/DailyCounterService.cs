...
public class DailyCounterService : ServerCoreRegisterServiceBase
{
    ...
    {
        Console.WriteLine($"[DEBUG]服务器({XFEServerCore.CoreServerName})天数统计服务已启动！");
    ...
    {
        runningDays++;
        Console.WriteLine($"[DEBUG]服务器({XFEServerCore.CoreServerName})自上次（{startTime}）启动以来，已稳定运行 {runningDays}天");
        try
        {
        ...
        {
            Console.WriteLine($"[ERROR]服务器({XFEServerCore.CoreServerName})保存日志时发生错误：{ex.Message}");
